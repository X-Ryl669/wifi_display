...
class ICal
{
    ...
    public function processRecurrences()
    {
        ...
        $rruleID = array(); $newEvents = array();
        foreach ($events as $anEvent) {
            if (isset($anEvent['RECURRENCE-ID']) && $anEvent['RECURRENCE-ID'] != '') {
                // TODO: Parse RANGE THISANDBEFORE and THISANDFUTURE
//                $anEvent['RRULE'] = $rruleID[$anEvent['UID'].' '.$anEvent['SEQUENCE']];
//                echo "Recurrence id found for rrule $anEvent[RRULE] and start $anEvent[DTSTART]\n";
            }
    ...
    public function sortEventsWithOrder($events, $sortOrder = SORT_ASC)
    {
        ...
        // Loop through all events by adding two new elements
        foreach ($events as $anEvent) {
            if (!$anEvent->unixTimestamp) {
                $anEvent->unixTimestamp = $this->iCalDateToUnixTimestamp($anEvent->dtstart);
            }

            if (!$anEvent->realDatetime) {
                $anEvent->realDatetime = date('d.m.Y', $anEvent->unixTimestamp);
            }
        ...
        foreach ($extendedEvents as $key => $value) {
            $timestamp[$key] = $value->unixTimestamp;
        }
